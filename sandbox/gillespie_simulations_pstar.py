...
# -*- coding: utf-8 -*-
"""
Created on Fri Jul 19 13:15:32 2024
...
"""
Code for a 2 component, single cell bursting model with flexible feedback functions. Implementation is an exact Gillespie scheme.
"""
import numpy as np
...
class Params:
    def __init__(self):
        ...
        self.KD_k_off = 10.0
        self.n = 2.0
        self.protein_activation_rate = 1.0
        self.protein_deactivation_rate = 1.0
        self.initial_activated_protein = 0.0
...
def simulate_multiple_copies(p):
    ...
    X = np.concatenate((np.array(p.initial_state), np.array([p.initial_mrna]), np.array([p.initial_protein]), np.array([p.initial_activated_protein])))
    if len(X.shape) == 1:
    ...
    while t < p.Tmax:
        ...
        X, tvec, t = update_multiple_copies(X, p, tvec, t, these_random_numbers)
        random_number_counter += 1

    return X, tvec, p


def update_multiple_copies(X, p, tvec, t, these_random_numbers):
    # get current values of the simulation
    current_X = X[-1]
    current_gene_states = current_X[:-3]
    current_mrna = current_X[-3]
    current_protein = current_X[-2]
    current_activated_protein = current_X[-1]
    ...
    off_propensities = current_gene_states * (p.k_off0 + p.k_off1 * (current_activated_protein / p.KD_k_off) ** p.n * hill_function(current_activated_protein, p.KD_k_off, p.n))
    on_propensities = (1 - current_gene_states) *  (p.k_on0 + p.k_on1 * hill_function(current_activated_protein, p.KD_k_on, p.n))
    transcription_propensity = np.sum(current_gene_states) * (p.transcription_rate_0 + p.transcription_rate_1 * hill_function(current_activated_protein, p.KD_transcription_rate, p.n))
    ...
    protein_activation_propensity = p.protein_activation_rate * current_protein
    protein_deactivation_propensity = p.protein_deactivation_rate * current_activated_protein
    activated_protein_decay_propensity = p.protein_decay_rate * current_activated_protein
    ...
    all_rates = np.concatenate((off_propensities, 
                                ...
                                np.array([protein_activation_propensity]),
                                np.array([protein_deactivation_propensity]),
                                np.array([activated_protein_decay_propensity])))
    ...
    if reaction_id < n_genes:
        ...
        # protein activation
        current_protein -= 1  
        current_activated_protein += 1
    elif reaction_id == 2 * n_genes + 5:
        # protein deactivation
        current_protein += 1
        current_activated_protein -= 1
    elif reaction_id == 2 * n_genes + 6:
        # activated protein decay
        current_activated_protein -= 1
    

    X_out = np.concatenate((current_gene_states, np.array([current_mrna]), np.array([current_protein]), np.array([current_activated_protein])))
